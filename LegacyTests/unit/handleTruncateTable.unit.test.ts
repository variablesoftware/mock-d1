...
describe('handleTruncateTable', () => {
  ...
  it('throws on malformed TRUNCATE TABLE statement (!tableMatch)', () => {
    const db = newDb();
    expect(() => handleTruncateTable('TRUNCATE TABLE', db)).toThrow('Malformed TRUNCATE TABLE statement.');
    expect(() => handleTruncateTable('TRUNCATE', db)).toThrow('Malformed TRUNCATE TABLE statement.');
    expect(() => handleTruncateTable('TRUNCATE TABLE ', db)).toThrow('Malformed TRUNCATE TABLE statement.');
  });
});
