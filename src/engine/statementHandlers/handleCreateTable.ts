...
export function handleCreateTable(
  ...
  if (/^CREATE\s+[^\s]+/i.test(sql) && !/^CREATE\s+TABLE/i.test(sql)) {
    log.error("Malformed CREATE (not CREATE TABLE)", { sql });
  ...
  try {
    ...
    try {
      ...
      log.error("Malformed CREATE TABLE (table name extraction failed)", { sql, err });
      throw d1Error('UNSUPPORTED_SQL');
    }
    ...
    if (!colMatch || typeof colMatch[3] !== 'string') {
      log.error("Malformed CREATE TABLE (regex failed or missing parens)", { sql, colMatch });
    ...
    if (colSection === '') {
      log.info("Created empty table (no columns, colSection empty string)", {
        sql,
        colMatch,
        colSection,
        tableKey
